#!/usr/bin/env bash
# 고아 줄 검사 — 줄 끝에 한두 글자만 남았는지 (design.md §4.10)
#
#   _scripts/check-orphans.sh                                   전체 슬라이드
#   _scripts/check-orphans.sh 2026-1학기_.../slides/02-x.html    지정한 것만
#
...
PUPPETEER_PATH="$PUP" node "$ROOT/_scripts/check-orphans.mjs" "${URLS[@]}"
